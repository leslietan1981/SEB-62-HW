// HW of unit 1-5 JavaScript Array Iterator Methods Lab Exercise
...
// // // use the delete to remove the keys year and passed before return into array inventorNames
...
// // try to write less with arrow expression
// let inventorNamedAda = inventors.find((adaFinder) =>
//   adaFinder.first === "Ada" ? true : false,
// );
...
// // use temperate literal as return
// let firstLast = people.map((peopleName) => {
//   const peopleArray = peopleName.split(", ");
...
// //   if (age >= 18) {
// //     console.log(`${developer.name} checked, he is ${age} years old`); // this line is optional, for checking purpose only
// //     return true;
// //   } else {
// //     console.log(`${developer.name} checked, he is ${age} years old`); // this line is optional, for checking purpose only
// //     return false;
...
// // Try ternary
// let isAdultPresent = devs.some((developer) =>
//   new Date().getFullYear() - developer.year >= 18 ? true : false,
// );
...
// // try ternary
// let isEveryone19OrOlder = devs.every((developer) =>
//   new Date().getFullYear() - developer.year >= 19 ? true : false,
// );
...
// // try to shorter
// let commentById = comments.find((findId) => findId.id === 823423);
...
// // Complete the exercise in the space below:
// let idx = comments.findIndex((index) => index.id === 123523);
...
// let addition = (total, inventor) => {
//   const yearsLived = inventor.passed - inventor.year;
//   return total + yearsLived;
// };

// // note that must have the zero for initialization, if not the default will be the very 1st object
// let totalYearsLived = inventors.reduce(addition, 0);
...
const checkTravel = (total, method) => {
  if (total[method]) {
    //if travelmethod found, add 1
    total[method] += 1;
  } else {
    //if travelmethod not found, start counting
    total[method] = 1;
  }
  return total;
};

let travelMethodCounts = travelMethods.reduce(checkTravel, {});
