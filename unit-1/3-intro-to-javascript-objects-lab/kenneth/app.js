...
*/
game.party.push(pokemon[117]);
game.party.push(pokemon[118]);
game.party.push(pokemon[119]);
console.log(game.party);
...
*/
// game.party.push(pokemon[3].starter);
// console.log(pokemon[3].name);
pokemon.splice(3, 1, pokemon[4]);
console.log(pokemon);
...
// const groupedByType = pokemon.reduce((acc, curr) => {
//   const { type } = curr;
...
//   return acc;
// }, {});
const groupByPokemonType = {};
pokemon.forEach((obj) => {
  const { type } = obj;
  if (!groupByPokemonType[type]) {
    groupByPokemonType[type] = [];
  } else {
    groupByPokemonType[type].push(obj);
  }
});

console.dir(groupByPokemonType, { maxArrayLength: null });
